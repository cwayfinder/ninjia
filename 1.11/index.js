...
function formatKey(key) {
  ...
  if (isInt(key)) {
    result = '[' + key + ']';
  } else if (!isValidVariableName(key)) {
    result = '["' + key + '"]';
  }
...
function search(needle, haystack) {
  ...
  Object.keys(haystack).forEach(function (key) {
    paths.push(formatKey(key));
    values.push(haystack[key]);
  });
  ...
  while (values.length) {
    ...
    if (firstValue === needle) {
      return firstPath;
    } else if (typeof firstValue === 'object') {
      Object.keys(firstValue).forEach(function (key) {
        paths.push(joinKey(firstPath, formatKey(key)));
        values.push(firstValue[key]);
      });
    }
  }
...
console.log(search(5, { a: [1, 2, { s: 4, c: { u: 5 } }], ss: 9 })); // 'a[2].c.u'
console.log(search(3, { '-bad-path': 3 })); // '["-bad-path"]'
